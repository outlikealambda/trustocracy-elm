...
Elm.User.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var Model = F2(function (a,b) {    return {name: a,id: b};});
   var init = F2(function (name,userid) {
      return A2(Model,name,userid);
   });
...
Elm.Relationship = Elm.Relationship || {};
Elm.Relationship.make = function (_elm) {
   "use strict";
   _elm.Relationship = _elm.Relationship || {};
   if (_elm.Relationship.values) return _elm.Relationship.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var getCircleType = function (relationship) {
      var _p0 = relationship;
      switch (_p0)
      {case "TRUSTS_EXPLICITLY": return "circle-explicit";
         case "TRUSTS": return "circle-regular";
         default: return "circle-unknown";}
   };
   var view = function (relationship) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class(getCircleType(relationship))]),
      _U.list([]));
   };
   return _elm.Relationship.values = {_op: _op,view: view};
};
Elm.Neighbor = Elm.Neighbor || {};
Elm.Neighbor.make = function (_elm) {
   "use strict";
   _elm.Neighbor = _elm.Neighbor || {};
   if (_elm.Neighbor.values) return _elm.Neighbor.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Relationship = Elm.Relationship.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   ...
   var view = function (n) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("single-line")]),
      _U.list([$Relationship.view(n.relationship)
              ,A2($Html.span,_U.list([]),_U.list([$Html.text(n.user.name)]))
              ,A2($Html.span,_U.list([]),_U.list([$Html.text(":")]))]));
   };
   var Model = F2(function (a,b) {
      return {user: a,relationship: b};
   });
   var decoder = A3($Json$Decode.object2,
   Model,
   A2($Json$Decode._op[":="],"friend",$User.decoder),
   A2($Json$Decode._op[":="],"rel",$Json$Decode.string));
   return _elm.Neighbor.values = {_op: _op
                                 ,view: view
                                 ,decoder: decoder
                                 ,Model: Model};
};
Elm.OpinionPath = Elm.OpinionPath || {};
Elm.OpinionPath.make = function (_elm) {
   "use strict";
   _elm.OpinionPath = _elm.OpinionPath || {};
   if (_elm.OpinionPath.values) return _elm.OpinionPath.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var view = function (op) {
      var relationships = A2($List.map,$Relationship.view,op.path);
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("single-line")]),
      _U.list([A2($Html.span,
              _U.list([$Html$Attributes.$class("friend")]),
              _U.list([$Html.text(op.friend.name)]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("single-line path")]),
              relationships)
              ,A2($Html.span,
              _U.list([$Html$Attributes.$class("opiner")]),
              _U.list([$Html.text(op.opiner.name)]))
              ,A2($Html.span,
              _U.list([$Html$Attributes.$class("opinion-id")]),
              _U.list([$Html.text($Basics.toString(op.opinionId))]))]));
   };
   var compareScore = F2(function (a,b) {
      var _p0 = A2($Basics.compare,a.score,b.score);
      switch (_p0.ctor)
      {case "LT": return $Basics.LT;
         case "GT": return $Basics.GT;
         default: return A2($Basics.compare,
           a.friend.name,
           b.friend.name);}
   });
   var compareOP = F2(function (a,b) {
      var _p1 = A2($Basics.compare,a.opiner.name,b.opiner.name);
      switch (_p1.ctor)
      {case "LT": return $Basics.LT;
         case "GT": return $Basics.GT;
         default: return A2(compareScore,a,b);}
   });
   var Model = F5(function (a,b,c,d,e) {
      return {friend: a,path: b,opiner: c,opinionId: d,score: e};
   });
   ...
   A2($Json$Decode._op[":="],"opiner",$User.decoder),
   A2($Json$Decode._op[":="],"opinion",$Json$Decode.$int),
   A2($Json$Decode._op[":="],"score",$Json$Decode.$int));
   return _elm.OpinionPath.values = {_op: _op
                                    ,view: view
                                    ,decoder: decoder
                                    ,compareOP: compareOP
                                    ,Model: Model};
...
Elm.Main.make = function (_elm) {
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Neighbor = Elm.Neighbor.make(_elm),
   $OpinionPath = Elm.OpinionPath.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $String = Elm.String.make(_elm),
   $Task = Elm.Task.make(_elm),
   ...
   var errorString = function (error) {
      ...
      {case "Timeout": return "timeout";
         ...
         case "UnexpectedPayload": return _p0._0;
         default: return A2($Basics._op["++"],
           $Basics.toString(_p0._0),
           A2($Basics._op["++"],": ",_p0._1));}
   };
   var nearestOpinionsDecoder = A2($Json$Decode._op[":="],
   "paths",
   $Json$Decode.list($OpinionPath.decoder));
   var userInfo = A3($Json$Decode.object2,
   F2(function (v0,v1) {
      return {ctor: "_Tuple2",_0: v0,_1: v1};
   }),
   A2($Json$Decode._op[":="],"user",$User.decoder),
   A2($Json$Decode._op[":="],
   "neighbors",
   $Json$Decode.list($Neighbor.decoder)));
   var isNum = function (n) {
      return $Basics.not($String.isEmpty(n)) && A2($String.all,
      $Char.isDigit,
      n);
   };
   var lookupNearestOpinions = F2(function (user,topic) {
      var toUrl = isNum(user) && isNum(topic) ? $Task.succeed(A2($Basics._op["++"],
      ...
      A2($Basics._op["++"],
      topic,
      "/opinions"))))) : $Task.fail("User-Topic combo is not valid");
      return A2($Task.andThen,
      toUrl,
      function (_p1) {
         return A2($Task.mapError,
         errorString,
         A2($Http.get,nearestOpinionsDecoder,_p1));
      });
   });
   var lookupNeighbors = function (rawId) {
      var toUrl = isNum(rawId) ? $Task.succeed(A2($Basics._op["++"],
      "http://localhost:3714/api/user/",
      rawId)) : $Task.fail("Please enter a valid user");
      return A2($Task.andThen,
      toUrl,
      function (_p2) {
         return A2($Task.mapError,
         errorString,
         A2($Http.get,userInfo,_p2));
      });
   };
   var results = $Signal.mailbox($Result.Err("Please enter a valid user"));
   var topicResults = $Signal.mailbox($Result.Err("Enter a topic ID"));
   var topicId = $Signal.mailbox("");
   var userId = $Signal.mailbox("");
   ...
   A2($Signal.map,lookupNeighbors,userId.signal)));
   var requestsNearest = Elm.Native.Task.make(_elm).performSignal("requestsNearest",
   A2($Signal.map,
   function (task) {
      return A2($Task.andThen,
      $Task.toResult(task),
      $Signal.send(topicResults.address));
   },
   A3($Signal.map2,
   lookupNearestOpinions,
   userId.signal,
   topicId.signal)));
   var viewNeighbors = function (neighbors) {
      return A2($Html.div,
      _U.list([]),
      A2($List.map,$Neighbor.view,neighbors));
   };
   ...
   var viewUser = F2(function (string,result) {
      var messages = function () {
         ...
         if (_p3.ctor === "Err") {
               ...
               _U.list([A2($Html.span,
                       ...
                       _p4.name,
                       A2($Basics._op["++"],", ",$Basics.toString(_p4.id))))]))
                       ,A2($Html.div,
                       _U.list([]),
                       A2($List.map,$Neighbor.view,_p3._0._1))]))]);
            }
      }();
      var field = A2($Html.input,
      _U.list([$Html$Attributes.placeholder("User ID")
              ,$Html$Attributes.value(string)
              ,A3($Html$Events.on,
              "input",
              $Html$Events.targetValue,
              $Signal.message(userId.address))]),
      _U.list([]));
      return A2($Html.div,
      _U.list([]),
      A2($List._op["::"],field,messages));
   });
   var viewNearest = F2(function (tid,result) {
      var paths = function () {
         var _p5 = result;
         if (_p5.ctor === "Err") {
               return _U.list([A2($Html.div,
               ...
               A2($List.map,
               $OpinionPath.view,
               A2($List.sortWith,$OpinionPath.compareOP,_p5._0)))]);
            }
      }();
      var field = A2($Html.input,
      _U.list([$Html$Attributes.placeholder("Topic ID")
              ,$Html$Attributes.value(tid)
              ,A3($Html$Events.on,
              "input",
              $Html$Events.targetValue,
              $Signal.message(topicId.address))]),
      _U.list([]));
      return A2($Html.div,
      _U.list([]),
      A2($List._op["::"],field,paths));
   });
   var view = F4(function (uid,result,tid,resultsTopic) {
      return A2($Html.div,
      _U.list([]),
      _U.list([css("css/style.css")
              ,A2(viewUser,uid,result)
              ,A2(viewNearest,tid,resultsTopic)]));
   });
   ...
   return _elm.Main.values = {_op: _op
                             ...
                             ,results: results
                             ,lookupNeighbors: lookupNeighbors
                             ,lookupNearestOpinions: lookupNearestOpinions
                             ,isNum: isNum
                             ,userInfo: userInfo
                             ,nearestOpinionsDecoder: nearestOpinionsDecoder
